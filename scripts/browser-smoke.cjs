...
(async () => {
  ...
  assert.ok(planBox.y + planBox.height * .79 < page.viewportSize().height, 'Draw action must bring route endpoints on screen');
  assert.ok(planBox.y + planBox.height * .65 > 68, 'Route start must clear the sticky header');
  await page.screenshot({ path: path.join(shots, 'muster-draw-input-check.png'), fullPage: false });
